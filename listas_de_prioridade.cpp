...
Heap::Heap(int n, int dados[]) :
  S(dados, dados + n) {          //(arg1) = inicio da copia, (arg2) = parada da copia
    int i;
    for(i = n/2 - 1; i >= 0; i--)
    {
      desce(i);
...
void Heap::desce(int i) {
  ...
  if(e < n && S[e] > S[i])   //testa se e é menor que o tamanho do vetor e se o vetor na posição e é maior que na posiçao i (filho essquerdo maior que o pai)
  {
    maior = e;
  }
  ...
  if(d < n && S[d] > S[maior])  //testa se d é menor que o tamanho do vetor e se o vetor na posição d é maior que na posiçao "maior" (filho direito maior que o "maior" até o momento)
  {
    maior = d;
  }

  if(maior != i)  //se encontrei algum filho maior que o pai
  {
    troca(i, maior);
    desce(maior);  //chamo desce recursivamente no filho que foi trocado até o fim dos niveis da arvore
  }
...
int Heap::extrai_maxima() {
  ...
  if(n > 0)
  {
    ...
    S[0] = S[n-1]; //ou S.back()
    S.pop_back(); //remove o ultimo elemento da arvore
    desce(0);     //reorganiza a arvore
    return maior;
  }
